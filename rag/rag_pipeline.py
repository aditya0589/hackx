...
class RAGPipeline:
    def __init__(self, embedding_dim=768, cache_dir="cache"):
    ...
    def _get_document_hash(self, link):
        """Generate a hash for the document to use as cache key."""
    ...
    def _load_from_cache(self, doc_hash):
        ...
        if os.path.exists(cache_path):
            try:
                with open(cache_path, 'rb') as f:
                    ...
                    print(f"[INFO] Loaded document from cache: {cache_path}")
                    return True
            except Exception as e:
                print(f"[WARNING] Failed to load cache: {e}")
        return False

    def _save_to_cache(self, doc_hash):
        ...
        try:
            ...
            print(f"[INFO] Saved document to cache: {cache_path}")
        except Exception as e:
            print(f"[WARNING] Failed to save cache: {e}")

    def ingest_document(self, link, chunk_strategy='sentences', target_chunks=50, batch_size=50, max_workers=4, clear_existing=True, use_cache=True):
        """
        Ingest a document with performance optimizations and caching.
        ...
        Args:
            ...
            use_cache: Whether to use caching for document ingestion
        """
        start_time = time.time()
        
        # Generate document hash for caching
        doc_hash = self._get_document_hash(link)
        
        # Try to load from cache first
        if use_cache and self._load_from_cache(doc_hash):
            total_time = time.time() - start_time
            print(f"[INFO] Document loaded from cache in {total_time:.2f}s")
            return
